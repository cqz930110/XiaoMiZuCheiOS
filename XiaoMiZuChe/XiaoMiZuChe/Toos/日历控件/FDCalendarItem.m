...
- (UILabel *)dayLabel {
    if (!_dayLabel) {
        _backView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 36, 36)];
        _backView.layer.masksToBounds = YES;
        _backView.layer.cornerRadius  = 18.f;
        _backView.center = CGPointMake(self.bounds.size.width / 2.f, self.bounds.size.height / 2.f +3.f);
        [self.contentView addSubview:_backView];
        
        _dayLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, 18, 18)];
        _dayLabel.textAlignment = NSTextAlignmentCenter;
        _dayLabel.adjustsFontSizeToFitWidth = YES;
        _dayLabel.font = [UIFont systemFontOfSize:15];
        _dayLabel.center = CGPointMake(self.bounds.size.width / 2.f, self.bounds.size.height / 2.f -4.f);
...
//            cell.backgroundColor = [UIColor clearColor];
//            cell.layer.cornerRadius = 20;//cell.frame.size.height / 2;
