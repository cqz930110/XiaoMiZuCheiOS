...
- (void)purchaseItem:(NSString *)identifier
{
    ...
    {
        DLog(@"1.不能处理付款-->SKPaymentQueue canMakePayments NO");
    ...
    DLog(@"1.可以处理付款-->请求产品信息...%@", identifier);
...
- (void)productsRequest:(SKProductsRequest *)request didReceiveResponse:(SKProductsResponse *)response
{
    ...
    DLog(@"myProduct: %@", myProduct);
    
    if (myProduct.count == 0)
    {
        DLog(@"2.失败-->无法获取产品信息，购买失败。invalidProductIdentifiers = %@", response.invalidProductIdentifiers);
...
{
    DLog(@"4.成功-->结束交易 SKPaymentTransactionStatePurchased");
...
{
    DLog(@"4.成功-->重置交易 SKPaymentTransactionStateRestored");
...
{
    DLog(@"4.成功-->交易失败 SKPaymentTransactionStateRestored error.code:%d",(int)transaction.error.code);
